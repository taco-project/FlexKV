...
| `FLEXKV_MOONCAKE_MAX_MR_SIZE_BYTES` | int | 549755813888 | 单个 Mooncake external MR 的最大字节数。更大的可共享 HugePage pool 会按 KV block、mapping alignment 和 HugePage 的共同边界拆分注册，并在失败或退出时逆序注销 |
