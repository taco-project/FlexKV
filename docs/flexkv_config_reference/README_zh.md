...
| `FLEXKV_TRANSFER_PATH_OPT` | bool | 1 | 是否启用 CE 优化路径（五路自适应策略）。`0` = baseline（`PER_BLOCK` 逐块 memcpy，无优化，用于性能对照基准）；`1` = 优化路径（`CONTIG_DIRECT` / `SEGMENT_DIRECT` / `SEGMENT_SCATTER` / `GATHER_SCATTER` / `GATHER_DIRECT` 自动选择，**默认**）。仅在 `FLEXKV_USE_CE_TRANSFER_H2D` 或 `FLEXKV_USE_CE_TRANSFER_D2H` 为 1 时生效 |
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | `SEGMENT_DIRECT` 与 `GATHER_SCATTER` 的段数切换阈值（仅 `FLEXKV_TRANSFER_PATH_OPT=1` 时生效）。**对 LAYERFIRST 场景生效**（非连续段数 ≤ 阈值时走 `SEGMENT_DIRECT`，否则走 `GATHER_SCATTER`）。BLOCKFIRST 场景走 `GATHER_DIRECT`，其内部分别在「整块直传」与「逐块拷贝」两种实现间切换（取决于是否一次性传输整层），其中逐块实现可启用 `cudaMemcpy2DAsync` 快路径（受 `FLEXKV_ENABLE_CE_MEMCPY2D` 控制）；`segment_threshold` 还用于 `SEGMENT_SCATTER` / `GATHER_SCATTER` 的 `cudaMemcpy2DAsync` 分支选择。可使用 `microbenchmark_ce_threshold.py` 找到工作负载的最优值 |
