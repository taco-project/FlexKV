...
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | Path 1 与 Path 2 的段数切换阈值（仅 auto 模式）。非连续段数 ≤ 阈值时走 Path 1，否则走 Path 2（gather/scatter pipeline）。可使用 `microbenchmark_ce_overhead.py --mode threshold` 找到工作负载的最优值 |
| `FLEXKV_TRANSFER_PINGPONG` | bool | 1 | 是否为 Path 1/Path 2 的 staging 传输启用 ping-pong 重叠（BLOCKFIRST 布局）。启用后 GPU D2H 第 L+1 层与 CPU scatter 第 L 层重叠执行，隐藏 CPU memcpy 延迟。调试或 CPU 已饱和时可关闭 |
