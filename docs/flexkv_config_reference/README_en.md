...
| `FLEXKV_MOONCAKE_MAX_MR_SIZE_BYTES` | int | 549755813888 | Maximum size of one Mooncake external MR. It can also be set as `mooncake_max_mr_size_bytes` in YAML/JSON; use `2147483648` for a 2 GiB transport limit. Larger pools are split without crossing KV-block boundaries. When the block/alignment period exceeds the MR limit, only derived sub-MR alignment is relaxed; partial failures and shutdown unregister regions in reverse order |
