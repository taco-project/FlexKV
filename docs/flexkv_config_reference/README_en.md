...
| `FLEXKV_MOONCAKE_MAX_MR_SIZE_BYTES` | int | 549755813888 | Maximum size of one Mooncake external MR. A larger shareable HugePage pool is registered in regions aligned to the KV block, mapping alignment, and HugePage size; partial failures and shutdown unregister regions in reverse order |
