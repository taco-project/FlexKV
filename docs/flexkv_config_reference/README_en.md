...
| `FLEXKV_TRANSFER_SEGMENT_THRESHOLD` | int | 8 | Segment count threshold for Path 1 vs Path 2 selection (auto mode only). When the number of non-contiguous segments ≤ threshold, Path 1 is used; otherwise Path 2 (gather/scatter pipeline). Use `--mode threshold` in `microbenchmark_ce_overhead.py` to find the optimal value for your workload |
| `FLEXKV_TRANSFER_PINGPONG` | bool | 1 | Enable ping-pong overlap for Path 1/Path 2 staging transfers (BLOCKFIRST layout). When enabled, GPU D2H of layer L+1 overlaps with CPU scatter of layer L, hiding CPU memcpy latency. Disable for debugging or when CPU is saturated |
