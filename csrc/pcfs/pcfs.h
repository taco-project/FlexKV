...
class Pcfs {
public:
    Pcfs(const std::string& fsid, uint32_t port, const std::string& ip, bool reserve, const uint64_t parent_nodeid);
    ~Pcfs();
    ...
    bool read(uint64_t file_nodeid, uint64_t offset, char* buffer, size_t size, int thread_id);
    //bool mkdir(const std::string& parent_dir, const std::string& dirname, uint64_t& dir_nodeid);
    //bool lookup(const std::string& parent_dir, const std::string& name, uint64_t& nodeid);
