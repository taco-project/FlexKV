...
std::vector<uint8_t> export_memory_handle(void *ptr, size_t size,
  ...
  CUmemGenericAllocationHandle handle;
  check_cu_result(cuMemRetainAllocationHandle(&handle, (void *)base_addr),
                  "Failed to retain allocation handle");

  ipc_sharable_cu_handle shareable_handle;
  check_cu_result(
      cuMemExportToShareableHandle(&shareable_handle.fd, handle,
...
void *import_memory_handle(const std::vector<uint8_t> &handle_data,
  ...
  ipc_sharable_cu_handle shareable_handle;
  std::memcpy(&shareable_handle, handle_data.data() + sizeof(MemoryHandle),
              sizeof(ipc_sharable_cu_handle));

  CUmemGenericAllocationHandle imported_handle;
  check_cu_result(cuMemImportFromShareableHandle(
                      &imported_handle, (void *)(uintptr_t)shareable_handle.fd,
                      CU_MEM_HANDLE_TYPE_POSIX_FILE_DESCRIPTOR),
                  "Failed to import shareable handle");

  void *reserved_addr = nullptr;
  check_cu_result(
      cuMemAddressReserve(reinterpret_cast<CUdeviceptr *>(&reserved_addr),
                          mem_handle.size,
                          0, // alignment
                          0, // addr
                          0  // flags
  ...
  check_cu_result(cuMemMap(reinterpret_cast<CUdeviceptr>(reserved_addr),
                           mem_handle.size,
                           0, // offset
                           imported_handle,
                           0 // flags
  ...
  check_cu_result(cuMemSetAccess(reinterpret_cast<CUdeviceptr>(reserved_addr),
                                 mem_handle.size, &access_desc,
