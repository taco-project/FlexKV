      ...
      try {
        ...
        if (is_mla) {
          ...
          if (mode == "sharded") {
            ...
            // Each rank writes to its own CPU region [i*num_blocks, (i+1)*num_blocks).
            // Encode rank offset into cpu_block_ids instead of cpu_startoff,
            // so strides don't need adjustment for total_blocks.
            cpu_startoff_inside_chunks = 0;
            gpu_startoff_inside_chunks = 0;
            chunk_size = gpu_chunk_sizes_in_bytes_[i];
        ...
        // For all_write, create per-rank cpu_block_ids with rank offset
        int64_t *effective_cpu_block_ids = cpu_block_ids;
        std::vector<int64_t> rank_cpu_ids;
        if (is_mla && mode == "all_write" && num_gpus_ > 1) {
          rank_cpu_ids.resize(num_blocks);
          for (int b = 0; b < num_blocks; b++) {
            rank_cpu_ids[b] = cpu_block_ids[b] + (int64_t)i * num_blocks;
          }
          effective_cpu_block_ids = rank_cpu_ids.data();
        }
        ...
        case BackendType::VLLM:
          flexkv::transfer_kv_blocks<BackendType::VLLM>(
              ...
              effective_cpu_block_ids, cpu_ptr, cpu_kv_stride_in_bytes,
