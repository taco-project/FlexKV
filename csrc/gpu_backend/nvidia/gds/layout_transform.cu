 ...
 *
 * Vendor-scoped layout transform kernel. Moved from
 * csrc/gds/layout_transform.cu during P3.
 *
 * GDS is NVIDIA-only (cuFile has no ROCm counterpart), so we keep
 * cudaStream_t / cudaGetDevice / cudaOccupancy* as-is.
 */
