...
SD-count upper bound is ``pp_node_count × tp_node_count`` (≤ 2 in the
current deployment, ≤ 4 in the future-extension scope ``pp_node_count=2 ×
tp_node_count=2``).
...
* :meth:`SharingDomainKey.enumerate_peers` returns exactly
  ``pp_node_count × tp_node_count`` unique SDs for every shape;
...
* :func:`graph_needs_gpu_clear` returns True iff the peer's
  ``pp_node_idx`` differs from self's (TP-node dim alone keeps the same
  slot_mapping, so no GPU clear is required);
...
def _sd(ppn_idx=0, ppn_count=1, tpn_idx=0, tpn_count=1,
        is_nsa=False) -> SharingDomainKey:
    """Build an SD key with the simplified, node-granularity PP schema."""
    return SharingDomainKey(
        model_id=MODEL_ID,
        pp_node_idx=ppn_idx, pp_node_count=ppn_count,
...
SHAPES = [
    (1, 1, "1-SD degenerate"),
    (2, 1, "pp_node_count=2 (current scope)"),
    (1, 2, "cross-node TP=2 (current scope)"),
    (2, 2, "pp_node_count=2 × cross-node TP=2 (future-extension upper bound, 4 SDs)"),
    # Stress: confirm no hidden caps.  Real deployments never go here.
    (4, 1, "pp_node_count=4 (stress)"),
    (1, 8, "tp_node_count=8 (stress)"),
    (4, 4, "pp_node_count=4 × tp_node_count=4 (stress, 16 SDs)"),
]
...
# ===========================================================================
@pytest.mark.parametrize("ppn,tpn,label", SHAPES)
def test_enumerate_peers_is_complete_cartesian_product(ppn, tpn, label):
    """``enumerate_peers`` must return every SD in the instance exactly once
    — product count = ppn × tpn, no duplicates.  CP no longer contributes."""
    master = _sd(ppn_count=ppn, tpn_count=tpn)
    peers = master.enumerate_peers()

    # Count matches the Cartesian product pp_node_count × tpn.
    assert len(peers) == ppn * tpn == master.total_sd_count()

    # Every (pp_node_idx, tp_node_idx) pair appears exactly once.
    pairs: Dict[Tuple[int, int], int] = {}
    for p in peers:
        pairs[(p.pp_node_idx, p.tp_node_idx)] = (
            pairs.get((p.pp_node_idx, p.tp_node_idx), 0) + 1
        )
    assert set(pairs.values()) == {1}, (
        f"[{label}] duplicate SDs detected in enumerate_peers: {pairs}"
    )
    # And every coordinate in range.
    for (pi, ti), _ in pairs.items():
        assert 0 <= pi < ppn
        assert 0 <= ti < tpn


@pytest.mark.parametrize("ppn,tpn,label", SHAPES)
def test_enumerated_sds_serialize_to_distinct_strings(ppn, tpn, label):
...
@pytest.mark.parametrize("ppn,tpn,label", SHAPES)
def test_serialize_deserialize_round_trip(ppn, tpn, label):
    master = _sd(ppn_count=ppn, tpn_count=tpn)
...
def test_no_cp_segment_in_any_serialized_sd():
    ...
    for ppn, tpn, _label in SHAPES:
        for p in _sd(ppn_count=ppn, tpn_count=tpn).enumerate_peers():
            assert ":cp" not in p.serialize(), (
                f"unexpected CP segment in serialized SD: {p.serialize()!r}"
            )


def test_total_sd_count_is_only_pp_node_count_times_tpn():
    """Regression: enabling CP at the model-config level must not multiply
    the SD count.  In the simplified schema only pp_node_count × tpn matters."""
    ...
    assert _sd(ppn_count=1, tpn_count=1).total_sd_count() == 1
    assert _sd(ppn_count=2, tpn_count=1).total_sd_count() == 2
    assert _sd(ppn_count=1, tpn_count=2).total_sd_count() == 2
    assert _sd(ppn_count=2, tpn_count=2).total_sd_count() == 4
...
def test_no_hardcoded_limit_on_any_single_dim():
    ...
    """
    assert len(_sd(ppn_count=16).enumerate_peers()) == 16
...
def test_stress_16_sds_enumerate_within_budget():
    ...
    import time
    master = _sd(ppn_count=4, tpn_count=4)
...
def test_graph_needs_gpu_clear_only_pp_node_dim_forces_clear():
    """Full 2-D grid sweep — under the simplified schema the rule is:
        ...
        clear?  ⇔  peer.pp_node_idx != self.pp_node_idx
...
# ===========================================================================
@pytest.mark.parametrize("ppn,tpn,label", [
    (2, 2, "current upper bound: 4 SDs"),
    (4, 4, "stress: 16 SDs"),
])
def test_each_sd_has_disjoint_redis_prefix(ppn, tpn, label):
    master = _sd(ppn_count=ppn, tpn_count=tpn)
...
def test_model_id_is_stable_across_calls():
    ...
    sd1 = _sd(ppn_count=4, tpn_count=4, ppn_idx=1, tpn_idx=2)
    sd2 = _sd(ppn_count=4, tpn_count=4, ppn_idx=1, tpn_idx=2)
...
# ===========================================================================
@pytest.mark.parametrize("ppn,tpn,label", SHAPES)
def test_exactly_one_master_in_any_instance(ppn, tpn, label):
    master = _sd(ppn_count=ppn, tpn_count=tpn)
    peers = master.enumerate_peers()
    ...
    m = masters[0]
    assert m.pp_node_idx == 0 and m.tp_node_idx == 0
