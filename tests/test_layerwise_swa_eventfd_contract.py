...
The authority on both is the GPU end-to-end layerwise suite, which reads real
eventfds off a real transfer.  These are the cheap, GPU-free guards that catch
the two edits most likely to break them silently -- a changed write value, and
a post moved from the per-model-layer marker to a per-region one.
"""
