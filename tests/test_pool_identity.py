...
def test_worker_op_carries_the_pool_across_the_pipe_as_the_member():
    ...
    so the member -- not merely an equal int -- has to survive the pipe.

    ``is`` rather than ``==`` on purpose: PoolId is an IntEnum, so ``==
    PoolId.SWA`` would also pass for a bare ``1`` that never interned, and the
    worker's ``self._pools[op.pool_id]`` lookup is what would then fail.
    """
