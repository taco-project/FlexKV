...
# Fixtures
# ---------------------------------------------------------------------------
def _sd(ppn_idx=0, ppn_count=1, tpn_idx=0, tpn_count=1, is_nsa=False):
...
class TestBuildHandles:
    ...
    def test_multi_sd(self):
        self_sd = _sd(ppn_count=2, tpn_count=2)  # 4 SDs total (pp_node_count × tpn)
        ...
        for peer in self_sd.enumerate_peers():
            ...
            endpoints[peer.serialize()] = _StubEndpoint(
                ip=f"10.0.{peer.pp_node_idx}.{peer.tp_node_idx}",
...
class TestFindEndpointForSd:
    ...
    def test_missing_raises(self):
        ...
        with pytest.raises(KeyError):
            find_endpoint_for_sd(cc, _sd(ppn_idx=1, ppn_count=2))
...
class TestGraphGpuClear:
    ...
    def test_pp_and_tp_both_differ(self, self_sd):
        peer = _sd(ppn_idx=1, ppn_count=2, tpn_idx=1, tpn_count=2)
        assert graph_needs_gpu_clear(self_sd, peer) is True
...
class TestMasterCoordinator:
    ...
    def test_on_remote_ready_completes(self):
        ...
        mc = MasterCoordinator(self_sd=sd, instance_id="inst-A")
        mc.expect_remotes(1)
        peer_sd = _sd(ppn_idx=1, ppn_count=2)
        msg = RemoteReadyMsg(
    ...
    def test_build_sd_to_nid(self):
        sd = _sd(ppn_count=2, tpn_count=2)  # 4 SDs total
        mc = MasterCoordinator(self_sd=sd, instance_id="inst-A")
        mc.expect_remotes(3)
    ...
    def test_expect_remotes_before_on_ready(self):
        ...
        msg = RemoteReadyMsg(
            sd_key=_sd(ppn_idx=1, ppn_count=2).serialize(),
        )
