...
* Current deployment (prefill crosses ≤ 2 nodes):
  - ``pp_node_count=2, tp_node_count=1`` — 2 SDs (PP-Remote lives on
    another node);
  - ``pp_node_count=1, tp_node_count=2`` — 2 SDs (TP-Remote lives on
    another node).
* Future-extension scope (not currently deployed):
  - ``pp_node_count=2, tp_node_count=2`` — 4 SDs.
...
Note on the rank→node collapse: under the simplified schema the PP axis
in the SD key is keyed by **physical node** (``pp_node_idx``), not by
PP rank.  Co-located PP ranks on the same machine fold into the same SD,
so a single-node ``pp_size=2`` deployment is *not* representable in this
test file — for that case the Master-side factory raises and the data
plane stays in legacy single-SD mode.
"""
...
class TestPhase2CrossNodeTPOnly:
    ...
    def test_enumerate_yields_2_distinct_sds(self):
        ...
        for p in peers:
            ...
            assert p.pp_node_idx == 0 and p.pp_node_count == 1
...
# ===========================================================================
# §4.10.1 future-extension row: pp_node_count=2 × tp_node_count=2 → 4 SDs
# ===========================================================================
class TestPhase2MaxConfig:
    SELF_SD = _sd(ppn_count=2, tpn_count=2)
    ...
    def test_enumerate_yields_4_distinct_sds(self):
        ...
        # Every (ppn, tpn) pair present exactly once.
        pairs: Set[Tuple[int, int]] = set()
        for p in peers:
            pairs.add((p.pp_node_idx, p.tp_node_idx))
        assert pairs == {(ppn, tpn) for ppn in range(2) for tpn in range(2)}

    def test_clear_decision_counts(self):
        """In a 4-SD instance with self at (ppn=0, tpn=0):

            peer (ppn=0, tpn=1) → TP-only differ → no clear
            peer (ppn=1, tpn=0) → ppn differs    → clear
            peer (ppn=1, tpn=1) → ppn differs    → clear
...
class TestBuildHandlesCurrentDeployment:
    def test_pp2_one_remote_handle(self):
        self_sd = _sd(ppn_count=2)
        peer = _sd(ppn_idx=1, ppn_count=2)
