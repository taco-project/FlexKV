"""
Distributed P2P Interleaved Test - Node B
...
def test_node_b():
    """Node B: Write odd IDs, Read 50-99"""
    print("=" * 80)
    print("Node B: Distributed P2P Interleaved Test")
    print("=" * 80)
    ...
    # 一个很重要的前提是：IMPORTANT
    # 驱逐开始时候，留下的缓冲blocks要足够大，作为期间request读写的buffer
    ...
    print("\n" + "=" * 80)
    print("Phase 2: Node B Writes ODD IDs [1, 3, 5, ..., 99]")
    print("=" * 80)
    
    # Node B writes odd IDs
    my_write_ids = list(range(1, num_total_request, 2))  # [1, 3, 5, ..., 99]
    print(f"Node B will write {len(my_write_ids)} requests: {my_write_ids[:10]}...")
    ...
    for req_id in my_write_ids:
        ...
        time.sleep(1)
        if (len([x for x in my_write_ids if x <= req_id]) % 10) == 0:
    ...
    print(f"Node B write completed in {write_elapsed:.2f} seconds")
    ...
    sync_wait_time = 20.0
    print(f"Waiting {sync_wait_time} seconds for Node A's index to sync...")
    time.sleep(sync_wait_time)
    print("Sync wait completed")
    
    print("\n" + "=" * 80)
    print("Phase 4: Node B Reads [50-99] (25 local + 25 remote)")
    print("=" * 80)
    
    # Node B reads 50-99
    my_read_ids = list(range(num_total_request // 2, num_total_request))  # [50, 51, 52, ..., 99]
    print(f"Node B will read {len(my_read_ids)} requests")
    print(f"  Expected local hits:  {len([x for x in my_read_ids if x % 2 == 1])} (odd IDs)")
    print(f"  Expected remote hits: {len([x for x in my_read_ids if x % 2 == 0])} (even IDs from Node A)")
    ...
    for req_id in my_read_ids:
        ...
        if ((req_id - 50) + 1) % 10 == 0:
            print(f"  Read {(req_id - 50) + 1}/{len(my_read_ids)} requests")
