...
* Layerwise + mooncake coexistence: mooncake lanes stay standalone and do NOT
  get folded into the LAYERWISE op.
"""
...
class TestLayerwiseMooncakeCoexistence:
    def test_mooncake_r2h_stays_standalone_with_layerwise_h2d(self):
        ...
        # Batch end is the VIRTUAL sink joining LAYERWISE + REMOTE2H.
        sinks = _find_virtual(merged)
        assert len(sinks) == 1
        sink = sinks[0]
        assert end == sink.op_id
        assert lw_ops[0].op_id in sink.predecessors
        assert r2h_ops[0].op_id in sink.predecessors
    ...
    def test_swa_mooncake_r2h_stays_standalone_with_layerwise(self):
        ...
        # end is the VIRTUAL sink.
        (sink,) = _find_virtual(merged)
        assert end == sink.op_id
        assert lw_ops[0].op_id in sink.predecessors
        assert swa_r2h_ops[0].op_id in sink.predecessors
