...
def update_default_config_from_user_config(rank_info: RankInfo,
    ...
    # Block size (bytes) used to translate the user's cpu_cache_gb / ssd_cache_gb
    # quota into a block count.
    #
    # Caveat (single-node PP):
    #   PR #171 made the per-node CPU pool cover ALL pp_stages on this node
    #   (num_layers_on_node, not num_layers_per_pp_stage). If we still size
    #   one block as 'per-pp-stage layers', num_cpu_blocks would be inflated by
    #   pp_size, and the actual pinned CPU memory would end up pp_size * cpu_cache_gb.
    #   For nnodes==1 the per-node pool is always the full model, so use the full
    #   layer count up-front. Cross-node PP keeps the per-stage sizing because
    #   each node only holds its own slice.
