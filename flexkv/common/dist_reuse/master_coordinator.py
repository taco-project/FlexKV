...
def graph_needs_gpu_clear(self_sd: SharingDomainKey, peer_sd: SharingDomainKey) -> bool:
    ...
    * **Different ``pp_node_idx``** → True — the peer's scheduler sees a
      different slot_mapping for the same task because the peer node owns a
      different layer range of the pipeline.
    ...
    * **Same SD as self** → False — it's the local graph.
    ...
    Note on the rank→node collapse: under the simplified schema, two PP
    ranks co-located on the same physical node share the same ``sd_key``
    (same ``pp_node_idx``).  Such co-located peers are not enumerated as
    remotes by :meth:`SharingDomainKey.enumerate_peers` so this function
    is never asked about them, which is the desired behavior — same-node
    cross-PP routing is a pool-layout concern, not a graph-clear concern.
    """
    if peer_sd == self_sd:
        return False
    return peer_sd.pp_node_idx != self_sd.pp_node_idx
